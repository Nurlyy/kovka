...
class RegaliaController extends Controller
{
    ...
    public function updateRegalia(Request $request){
        if ($request->hasFile('image')) {
            $regalia_id =  $request->id;
            $regalia = Regalia::findById($regalia_id);
            $imageName = $regalia->image;
            if('/uploads/regalia/'.$request->image->getClientOriginalName().'.'.$request->image->getClientOriginalExtension() != $regalia->image){
                unlink(public_path('uploads/regalia').'/'.$regalia->image);
                $imageName = time() . '.' . $request->image->getClientOriginalExtension();
                $request->image->move(public_path('uploads/regalia'), $imageName);
    ...
    {
        if(isset($request->id)){
            $regalia = Regalia::findById($request->id);
        }else{
            $regalia = Regalia::find()->all();
        }
        return response()->json(['regalia' => $regalia]);
    }
}
